...
Py2HWSW will self-install when nix-shell is run on a directory that contains the
[py2hwsw default.nix
file](https://github.com/IObundle/py2hwsw/blob/main/py2hwsw/lib/default.nix). All
dependencies will also be installed.

In this directory, [default.nix](https://github.com/IObundle/iob-soc-ibex/blob/main/default.nix) will automatically call the [py2hwsw default.nix
file](https://github.com/IObundle/py2hwsw/blob/main/py2hwsw/lib/default.nix). So, you can just use a Nix command with `nix-shell` in this directory, or run a Makefile target like `make setup`, and everything will be installed.
...
This is not advised for any IOBundle system, but specially not advised for this specific system, since this repository is setup so that it uses one Nix environment to run [Py2HWSW](https://github.com/IObundle/py2hwsw/), and a different one to run [Ibex](https://github.com/lowRISC/ibex). Basically, there was already a Nix environment built with all the CPU dependencies in [Ibex-Demo-System](https://github.com/lowRISC/ibex-demo-system), so we used that.
...
## Setup Ibex by generating and copy RTL files
...
First, we use the default [Nix-Shell Environment](https://github.com/IObundle/iob-soc-ibex/blob/main/default.nix) (together with [Py2HWSW](https://github.com/IObundle/py2hwsw/)) to create the Build Directory, generate all the Verilog files of the SoC's system (peripherals, interconnects, testbenches, etc).
